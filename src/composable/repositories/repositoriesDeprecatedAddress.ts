/*
 * R Depot
 *
 * Copyright (C) 2012-2025 Open Analytics NV
 *
 * ===========================================================================
 *
 * This program is free software: you can redistribute it and/or modify
 * it under the terms of the Apache License as published by
 * The Apache Software Foundation, either version 2 of the License, or
 * (at your option) any later version.
 *
 * This program is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
 * Apache License for more details.
 *
 * You should have received a copy of the Apache License
 * along with this program. If not, see <http://www.apache.org/licenses/>
 *
 */

import { Technologies } from '@/enum/Technologies'
import getEnv from '@/utils/env'

export function useRepositoryDeprecated() {
  function deprecatedAddress(
    serverAddress: string,
    technology?: Technologies
  ) {
    if (
      getEnv('VITE_ADDRESS_DEPRECATION_WARNING') === 'false'
    ) {
      return false
    }
    if (technology === undefined) {
      return false
    }
    return technology === Technologies.Enum.R
      ? !serverAddress?.includes('/r/')
      : !serverAddress?.includes('/python/')
  }

  function deprecatedAddressTooltip(serverAddress: string) {
    return (
      !serverAddress?.includes('/r/') &&
      !serverAddress?.includes('/python/')
    )
  }

  function getNewServerAddress(
    serverAddress: string,
    technology?: Technologies
  ) {
    let address = serverAddress?.slice(
      0,
      serverAddress.lastIndexOf('/')
    )
    address +=
      technology === Technologies.Enum.R ? '/r' : '/python'
    return (
      address +
      serverAddress?.slice(serverAddress.lastIndexOf('/'))
    )
  }

  return {
    deprecatedAddress,
    deprecatedAddressTooltip,
    getNewServerAddress
  }
}
