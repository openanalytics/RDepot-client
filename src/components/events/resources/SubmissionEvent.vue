...
<template>
  ...
  <ul class="value" v-else>
    <li>
      ...
      <strong>{{
        event?.relatedResource?.packageBag?.name
      }}</strong>
    </li>
    <li>
      {{ $t('columns.package.version') }}:
      {{ event?.relatedResource?.packageBag?.version }}
    </li>
    <li>
    ...
    <li>
      ...
      {{
        event?.relatedResource?.packageBag?.repository?.name
      }}
    </li>
    <li>
      {{ $t('columns.state') }}:
      {{ event?.relatedResource?.state }}
    </li>
  </ul>
</template>
